...
abstract class TelemetryDatabase : RoomDatabase() {
    ...
    companion object {
        ...
        /**
         * v6 → v7: persist per-sample wire status fields (faultCode, flags).
         *
         * These are independent wire channels — there's nothing to recompute
         * them from — so they're NOT NULL DEFAULT 0 rather than nullable-with-
         * fallback like rpm/power. Pre-v7 rows read 0 (no fault, no flags),
         * exactly what decodeEntity produced before they were stored. Persisting
         * them makes replay / Trip Detail render the same telltales and faults
         * the live Drive page showed for the recorded frames.
         */
        internal val MIGRATION_6_7 = object : Migration(6, 7) {
            override fun migrate(db: SupportSQLiteDatabase) {
                db.execSQL("ALTER TABLE telemetry ADD COLUMN faultCode INTEGER NOT NULL DEFAULT 0")
                db.execSQL("ALTER TABLE telemetry ADD COLUMN flags INTEGER NOT NULL DEFAULT 0")
            }
        }

        /**
         * All migrations live in this list. Adding a new one is append-only —
         * future SOH / eco-score work will declare `Migration(7, 8)` above and
         * append it here.
         */
