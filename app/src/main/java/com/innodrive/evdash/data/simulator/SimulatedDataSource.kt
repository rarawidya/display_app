...
import com.innodrive.evdash.data.protocol.TelemetrySchema
...
class SimulatedDataSource(
    ...
    // Trip B integrates independently from a seeded baseline (proves @16 is a
    // distinct channel from trip A, which starts at 0 each session).
    private var tripBMetersAccum = 104_500.0
    ...
    private fun generateFrame(): ByteArray {
        ...
        val capnpPayload = TelemetrySchema.buildMessage {
            ...
            // @13 legacy alias mirrors trip A; @15/@16 are the real A/B channels.
            setTripMeters(tripMetersAccum.toInt())
            setTripAMeters(tripMetersAccum.toInt())
            setTripBMeters(tripBMetersAccum.toInt())
            // Board firmware (packed semver 1.5.0) + VOTOL controller, no OTA active —
            // exercises the fwVersion @17 / controllerType @23 display wiring.
            setFwVersion(SIM_FW_VERSION)
            setOtaState(0)
            setOtaProgress(0)
            setControllerType(0) // 0 = VOTOL EM-100
        }
    ...
    private companion object {
        // Packed semver 1.5.0 = (1<<16)|(5<<8)|0 — distinct from the DIS mock so the
        // telemetry-fwVersion-wins wiring is visible in a demo.
        const val SIM_FW_VERSION = (1 shl 16) or (5 shl 8) or 0
    }
}
